...
AddTurbo = function(net, type)
    ...
    if not DoesEntityExist(vehicle) or not turbo then
        return
    end
...
RegisterNetEvent('daeng_turbo:AddTurbo', AddTurbo)

Citizen.CreateThread(function()
    Wait(1000)
    local ret = json.decode(GetResourceKvpString('daeng_turbo') or '[]') or {}
    for k, v in pairs(ret) do
        if not turbos[k] then
            turbos[k] = {}
        end
        turbos[k].plate = k
    ...
    while true do
        ...
        SetResourceKvp('daeng_turbo', json.encode(turbodata))
    end
end)

function SaveTurbo(plate, turbo)
    local data = json.decode(GetResourceKvpString('daeng_turbo') or '[]') or {}
    data[plate] = turbo
    SetResourceKvp('daeng_turbo', json.encode(data))
end
...
Citizen.CreateThread(function()
    for v, k in pairs(Config.turbos) do
        ...
        RegisterUsableItem("turbo" .. turboname .. "", function(source)
            local xPlayer = GetPlayerFromId(source)
            if Config.jobonly and xPlayer.job.name ~= tostring(Config.turbojob) then
                print("not mech")
                return
            end
...
AddStateBagChangeHandler('bov' --[[key filter]] , nil --[[bag filter]] ,
    function(bagName, key, value, _unused, replicated)
        ...
        if DoesEntityExist(entity) then
            ...
            state.durability = state.durability - (0.05) * value.boost
            if state then
